...
const objModuleName: _type.ModuleSpecificObjectType = {
    ...
    ApiModule: 'Product',
    ApiType: _type.ApiType.FormData,
...
function AdditionalFunctionalityOnNewEntry() {
    _utils.GenericFunctionality.HtmlLogs(`AdditionalFunctionalityOnNewEntry`, `Process started!`, '');

}

function AdditionalFunctionalityOnBeforeEditEntry() {
    _utils.GenericFunctionality.HtmlLogs(`AdditionalFunctionalityOnBeforeEditEntry`, `Process started!`, '');
    let imageElement = document.getElementById(`ProductImage`)! as HTMLInputElement;
    let imagePreviewElement = document.getElementById(`ImagePreview`)! as HTMLImageElement;
    if (imageElement) {
        imageElement.value = '';
    }
    ...
    _utils.GenericFunctionality.HtmlLogs(`AdditionalFunctionalityOnBeforeEditEntry`, `Process end!`, '');
}
export class clsIndexPage {
    ...
    public static ModelSpecificEventBinding() {
        _utils.GenericFunctionality.HtmlLogs(`ModelSpecificEventBinding`, `Execution started!`, '');
        const imageLink = document.querySelector(`#UploadImage`)! as HTMLInputElement;

        if (imageLink) {
            _utils.GenericFunctionality.HtmlLogs(`ModelSpecificEventBinding`, `UploadImage element found!`, '');
            imageLink.addEventListener('change', function (event) {
                ...
                if (input.files && input.files[0]) {
                    ...
                    _utils.GenericFunctionality.HtmlLogs(`ModelSpecificEventBinding`, `Checking uploaded image fize size!`, `File size: ${fileSize / 1024} KB.`);
                    if ((fileSize / 1024) > 50) {
                        ...
                        reader.onload = function (e) {
                            ...
                            _utils.GenericFunctionality.HtmlLogs(`ModelSpecificEventBinding`, `Image set!`, '');
                        }
        ...
        _utils.GenericFunctionality.HtmlLogs(`ModelSpecificEventBinding`, `Execution completed!`, '');
    }
