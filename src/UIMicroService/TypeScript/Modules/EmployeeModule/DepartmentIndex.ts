...
const objModuleName: _type.ModuleSpecificObjectType = {
    ...
    CacheKey: "DepartmentModule",
    ApiModule: 'EmployeeDepartment',
    ApiType:_type.ApiType.Json,
    UseBrowserCache: true,
    UseRedisCache: true,
    BaseUrl: `https://localhost/ApiGateway`,
    RecordId: ``,
    AsideBarLink: `EmployeeDepartment-MasterModule`,
...
export class clsIndexPage {
    ...
    public static async FillDataOnIndexTable(result: object[] | any[]) {
        ...
        result.forEach((record) => {
            ...
            templateRow += `<p class="text-blue">Designations :&nbsp;<span class="text-black"><<DESIGNATIONS>></span></p>`;
            let designationList: string = '';
            record.designations.forEach((designation) => {
                designationList += `<li>${designation.name}</li>`;
            })
            templateRow = templateRow.replace(`<<DESIGNATIONS>>`, designationList)
        ...
        _utils.GenericFunctionality.HtmlLogs(`FillDataOnIndexTable`, `Binding data on index page completed.`, '');
