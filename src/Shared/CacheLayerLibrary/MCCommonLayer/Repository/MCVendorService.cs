...
namespace MCCommonLayer.Repository
{
    ...
    /// </summary>
    public class MCVendorService : IMCVendorContract
    {
        private readonly IDataLayerVendorContract _dlService;
        private ICacheContract _cacheData;
        private DateTimeOffset _expirationTime;
        private ApplicationSettings _applicationSettings;
        public MCVendorService(IDataLayerVendorContract dlService, ICacheContract _cache, ITrace trace)
        {
        ...
        private async Task UpdateMemoryCacheOnCRUDAsync(VendorModel modelRecord, string clientId, bool isNew)
        {
            (bool status, string message) operationStatus = await _cacheData.UpdateCacheAsync<VendorModel>("Index", modelRecord.Id, modelRecord, _expirationTime, isNew: isNew);
        ...
        public async Task<VendorModel> AddCacheAsync(VendorModel modelRecord, string clientId, bool useCache)
        {
        ...
        public async Task<EnumRecordStatus> DeleteCacheAsync(string id, VendorModel modelRecord, int _modificationInDays, bool useCache)
        {
            var record = await _dlService.DeleteAsync(modelRecord);
            if (useCache && record.RecordStatus == EnumRecordStatus.Deleted)
            {
                (bool status, string message) recordStatus = await _cacheData.RemoveFromCacheAsync<VendorModel>("Index", id, _modificationInDays, _expirationTime);
        ...
        public async Task<VendorModel?> GetCacheAsync(string id, int _modificationInDays, string clientId, bool useCache)
        {
            ...
            List<VendorModel>? models = await GetAllCacheAsync(_modificationInDays, clientId, useCache);
            ...
            else
            {
                ...
                if (models?.Count > 0)
                {
                    var temp = await _cacheData.AddCacheAsync<List<VendorModel>>(_cacheData.GenerateKeyForCache<VendorModel>("Index", clientId), models, _expirationTime);
                    if (temp.status == true)
                    {
                        return models.FirstOrDefault(m => m.Id == id);
                    }
        ...
        public async Task<List<VendorModel>?> GetAllCacheAsync(int _modificationInDays, string clientId, bool useCache)
        {
            List<VendorModel>? models = null;
            if (useCache)
                models = await _cacheData.ReadFromCacheAsync<VendorModel>("Index", _modificationInDays);
            ...
            else
            {
                ...
                if (models?.Count > 0)
                {
                    if (useCache)
                    {
                        var temp = await _cacheData.AddCacheAsync<ICollection<VendorModel>>(_cacheData.GenerateKeyForCache<VendorModel>("Index", clientId), models, _expirationTime);
                        if (temp.status == true)
                        {
                            return models;
                        }
        ...
        public async Task<VendorModel> UpdateCacheAsync(VendorModel modelRecord, string clientId, bool UseCache)
        {
