...
{
    /// <summary>
    /// Memory cache EmployeeQualification service
    /// </summary>
    public class MCEmployeeQualificationService : IMCEmployeeQualificationContract
    {
        private readonly IDataLayerEmployeeQualificationContract _dlService;
        private ICacheContract _cacheData;
        private DateTimeOffset _expirationTime;
        private ApplicationSettings _applicationSettings;
        public MCEmployeeQualificationService(IDataLayerEmployeeQualificationContract dlService, ICacheContract _cache, ITrace trace)
        {
        ...
        private async Task UpdateMemoryCacheOnCRUDAsync(EmployeeQualificationModel modelRecord, string clientId, bool isNew)
        {
        ...
        public async Task<EmployeeQualificationModel> AddCacheAsync(EmployeeQualificationModel modelRecord, string clientId, bool useCache)
        {
        ...
        public async Task<EnumRecordStatus> DeleteCacheAsync(string id, EmployeeQualificationModel modelRecord, int _modificationInDays, bool useCache)
        {
            EmployeeQualificationModel? record = await _dlService.DeleteAsync(modelRecord);
            if (useCache && record.RecordStatus == EnumRecordStatus.Deleted)
            {
                (bool status, string message) recordStatus = await _cacheData.RemoveFromCacheAsync<EmployeeQualificationModel>("Index", id, _modificationInDays, _expirationTime);
        ...
        public async Task<EmployeeQualificationModel?> GetCacheAsync(string id, int _modificationInDays, string clientId, bool useCache)
        {
            ...
            List<EmployeeQualificationModel>? models = await GetAllCacheAsync(_modificationInDays, clientId, useCache);
            EmployeeQualificationModel? model = null;
            if (models?.Count > 0)
            {
            ...
            else
            {
                ...
                if (models?.Count > 0)
                {
                    (bool status, string message) recordStatus = await _cacheData.AddCacheAsync(_cacheData.GenerateKeyForCache<EmployeeQualificationModel>("Index", clientId), models, _expirationTime);
        ...
        public async Task<List<EmployeeQualificationModel>?> GetAllCacheAsync(int _modificationInDays, string clientId, bool useCache)
        {
            List<EmployeeQualificationModel>? models = null;
            if (useCache)
                models = await _cacheData.ReadFromCacheAsync<EmployeeQualificationModel>("Index", _modificationInDays);
            ...
            else
            {
                ...
                if (models?.Count > 0)
                {
                    if (useCache)
                    {
                        (bool status, string message) recordStatus = await _cacheData.AddCacheAsync<ICollection<EmployeeQualificationModel>>(_cacheData.GenerateKeyForCache<EmployeeQualificationModel>("Index", clientId), models, _expirationTime);
                        if (recordStatus.status == true)
                        {
                            return models;
                        }
        ...
        public async Task<EmployeeQualificationModel> UpdateCacheAsync(EmployeeQualificationModel modelRecord, string clientId, bool UseCache)
        {
