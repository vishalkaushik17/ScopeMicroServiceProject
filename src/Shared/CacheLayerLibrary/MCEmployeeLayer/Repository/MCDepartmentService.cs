...
{
    /// <summary>
    /// Memory cache Department service
    /// </summary>
    public class MCDepartmentService : IMCDepartmentContract
    {
        private readonly IDataLayerDepartmentContract _dlService;
        private ICacheContract _cacheData;
        private DateTimeOffset _expirationTime;
        private ApplicationSettings _applicationSettings;
        public MCDepartmentService(IDataLayerDepartmentContract dlService, ICacheContract _cache, ITrace trace)
        {
        ...
        private async Task UpdateMemoryCacheOnCRUDAsync(DepartmentModel modelRecord, string clientId, bool isNew)
        {
        ...
        public async Task<DepartmentModel> AddCacheAsync(DepartmentModel modelRecord, string clientId, bool useCache)
        {
        ...
        public async Task<EnumRecordStatus> DeleteCacheAsync(string id, DepartmentModel modelRecord, int _modificationInDays, bool useCache)
        {
            DepartmentModel? record = await _dlService.DeleteAsync(modelRecord);
            if (useCache && record.RecordStatus == EnumRecordStatus.Deleted)
            {
                (bool status, string message) recordStatus = await _cacheData.RemoveFromCacheAsync<DepartmentModel>("Index", id, _modificationInDays, _expirationTime);
        ...
        public async Task<DepartmentModel?> GetCacheAsync(string id, int _modificationInDays, string clientId, bool useCache)
        {
            ...
            List<DepartmentModel>? models = await GetAllCacheAsync(_modificationInDays, clientId, useCache);
            ...
            else
            {
                ...
                if (models?.Count > 0)
                {
                    (bool status, string message) recordStatus = await _cacheData.AddCacheAsync(_cacheData.GenerateKeyForCache<DepartmentModel>("Index", clientId), models, _expirationTime);
        ...
        public async Task<List<DepartmentModel>?> GetAllCacheAsync(int _modificationInDays, string clientId, bool useCache)
        {
            List<DepartmentModel>? models = null;
            if (useCache)
                models = await _cacheData.ReadFromCacheAsync<DepartmentModel>("Index", _modificationInDays);
            ...
            else
            {
                ...
                if (models?.Count > 0)
                {
                    if (useCache)
                    {
                        (bool status, string message) recordStatus = await _cacheData.AddCacheAsync<ICollection<DepartmentModel>>(_cacheData.GenerateKeyForCache<DepartmentModel>("Index", clientId), models, _expirationTime);
                        if (recordStatus.status == true)
                        {
                            return models;
                        }
        ...
        public async Task<DepartmentModel> UpdateCacheAsync(DepartmentModel modelRecord, string clientId, bool UseCache)
        {
