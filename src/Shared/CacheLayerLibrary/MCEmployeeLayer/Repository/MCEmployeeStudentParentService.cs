...
{
    /// <summary>
    /// Memory cache EmployeeStudentParent service
    /// </summary>
    public class MCEmployeeStudentParentService : IMCEmployeeStudentParentContract
    {
        private readonly IDataLayerEmployeeStudentParentContract _dlService;
        private ICacheContract _cacheData;
        private DateTimeOffset _expirationTime;
        private ApplicationSettings _applicationSettings;
        public MCEmployeeStudentParentService(IDataLayerEmployeeStudentParentContract dlService, ICacheContract _cache, ITrace trace)
        {
        ...
        private async Task UpdateMemoryCacheOnCRUDAsync(EmployeeStudentParentModel modelRecord, string clientId, bool isNew)
        {
        ...
        public async Task<EmployeeStudentParentModel> AddCacheAsync(EmployeeStudentParentModel modelRecord, string clientId, bool useCache)
        {
        ...
        public async Task<EnumRecordStatus> DeleteCacheAsync(string id, EmployeeStudentParentModel modelRecord, int _modificationInDays, bool useCache)
        {
            EmployeeStudentParentModel? record = await _dlService.DeleteAsync(modelRecord);
            if (useCache && record.RecordStatus == EnumRecordStatus.Deleted)
            {
                (bool status, string message) recordStatus = await _cacheData.RemoveFromCacheAsync<EmployeeStudentParentModel>("Index", id, _modificationInDays, _expirationTime);
        ...
        public async Task<EmployeeStudentParentModel?> GetCacheAsync(string id, int _modificationInDays, string clientId, bool useCache)
        {
            ...
            List<EmployeeStudentParentModel>? models = await GetAllCacheAsync(_modificationInDays, clientId, useCache);
            EmployeeStudentParentModel? model = null;
            if (models?.Count > 0)
            {
            ...
            else
            {
                ...
                if (models?.Count > 0)
                {
                    (bool status, string message) recordStatus = await _cacheData.AddCacheAsync(_cacheData.GenerateKeyForCache<EmployeeStudentParentModel>("Index", clientId), models, _expirationTime);
        ...
        public async Task<List<EmployeeStudentParentModel>?> GetAllCacheAsync(int _modificationInDays, string clientId, bool useCache)
        {
            List<EmployeeStudentParentModel>? models = null;
            if (useCache)
                models = await _cacheData.ReadFromCacheAsync<EmployeeStudentParentModel>("Index", _modificationInDays);
            ...
            else
            {
                ...
                if (models?.Count > 0)
                {
                    if (useCache)
                    {
                        (bool status, string message) recordStatus = await _cacheData.AddCacheAsync<ICollection<EmployeeStudentParentModel>>(_cacheData.GenerateKeyForCache<EmployeeStudentParentModel>("Index", clientId), models, _expirationTime);
                        if (recordStatus.status == true)
                        {
                            return models;
                        }
        ...
        public async Task<EmployeeStudentParentModel> UpdateCacheAsync(EmployeeStudentParentModel modelRecord, string clientId, bool UseCache)
        {
