...
namespace MCEmployeeLayer.Repository
{
    ...
    /// </summary>
    public class MCBankService : IMCBankContract
    {
        private readonly IDataLayerBankContract _dlService;
        private ICacheContract _cacheData;
        private DateTimeOffset _expirationTime;
        private ApplicationSettings _applicationSettings;
        public MCBankService(IDataLayerBankContract dlService, ICacheContract _cache, ITrace trace)
        {
        ...
        private async Task UpdateMemoryCacheOnCRUDAsync(BankModel modelRecord, string clientId, bool isNew)
        {
        ...
        public async Task<BankModel> AddCacheAsync(BankModel modelRecord, string clientId, bool useCache)
        {
        ...
        public async Task<EnumRecordStatus> DeleteCacheAsync(string id, BankModel modelRecord, int _modificationInDays, bool useCache)
        {
            BankModel? record = await _dlService.DeleteAsync(modelRecord);
            if (useCache && record.RecordStatus == EnumRecordStatus.Deleted)
            {
                (bool status, string message) recordStatus = await _cacheData.RemoveFromCacheAsync<BankModel>("Index", id, _modificationInDays, _expirationTime);
        ...
        public async Task<BankModel?> GetCacheAsync(string id, int _modificationInDays, string clientId, bool useCache)
        {
            ...
            List<BankModel>? models = await GetAllCacheAsync(_modificationInDays, clientId, useCache);
            ...
            else
            {
                ...
                if (models?.Count > 0)
                {
                    (bool status, string message) recordStatus = await _cacheData.AddCacheAsync(_cacheData.GenerateKeyForCache<BankModel>("Index", clientId), models, _expirationTime);
        ...
        public async Task<List<BankModel>?> GetAllCacheAsync(int _modificationInDays, string clientId, bool useCache)
        {
            List<BankModel>? models = null;
            if (useCache)
                models = await _cacheData.ReadFromCacheAsync<BankModel>("Index", _modificationInDays);
            ...
            else
            {
                ...
                if (models?.Count > 0)
                {
                    if (useCache)
                    {
                        (bool status, string message) recordStatus = await _cacheData.AddCacheAsync<ICollection<BankModel>>(_cacheData.GenerateKeyForCache<BankModel>("Index", clientId), models, _expirationTime);
                        if (recordStatus.status == true)
                        {
                            return models;
                        }
        ...
        public async Task<BankModel> UpdateCacheAsync(BankModel modelRecord, string clientId, bool UseCache)
        {
