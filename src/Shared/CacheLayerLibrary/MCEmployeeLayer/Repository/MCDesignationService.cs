...
{
    /// <summary>
    /// Memory cache Designation service
    /// </summary>
    public class MCDesignationService : IMCDesignationContract
    {
        private readonly IDataLayerDesignationContract _dlService;
        private ICacheContract _cacheData;
        private DateTimeOffset _expirationTime;
        private ApplicationSettings _applicationSettings;
        public MCDesignationService(IDataLayerDesignationContract dlService, ICacheContract _cache, ITrace trace)
        {
        ...
        private async Task UpdateMemoryCacheOnCRUDAsync(DesignationModel modelRecord, string clientId, bool isNew)
        {
        ...
        public async Task<DesignationModel> AddCacheAsync(DesignationModel modelRecord, string clientId, bool useCache)
        {
        ...
        public async Task<EnumRecordStatus> DeleteCacheAsync(string id, DesignationModel modelRecord, int _modificationInDays, bool useCache)
        {
            DesignationModel? record = await _dlService.DeleteAsync(modelRecord);
            if (useCache && record.RecordStatus == EnumRecordStatus.Deleted)
            {
                (bool status, string message) recordStatus = await _cacheData.RemoveFromCacheAsync<DesignationModel>("Index", id, _modificationInDays, _expirationTime);
        ...
        public async Task<DesignationModel?> GetCacheAsync(string id, int _modificationInDays, string clientId, bool useCache)
        {

            if (!useCache)
            {
                //we are getting department master table here
                return await _dlService.GetAsync(id);
            }


            List<DesignationModel>? models = await GetAllCacheAsync(_modificationInDays, clientId, useCache);
            ...
            else
            {
                ...
                if (models?.Count > 0)
                {
                    (bool status, string message) recordStatus = await _cacheData.AddCacheAsync(_cacheData.GenerateKeyForCache<DesignationModel>("Index", clientId), models, _expirationTime);
        ...
        public async Task<List<DesignationModel>?> GetAllCacheAsync(int _modificationInDays, string clientId, bool useCache)
        {
            List<DesignationModel>? models = null;
            if (useCache)
                models = await _cacheData.ReadFromCacheAsync<DesignationModel>("Index", _modificationInDays);
            ...
            else
            {
                ...
                if (models?.Count > 0)
                {
                    if (useCache)
                    {
                        (bool status, string message) recordStatus = await _cacheData.AddCacheAsync<ICollection<DesignationModel>>(_cacheData.GenerateKeyForCache<DesignationModel>("Index", clientId), models, _expirationTime);
                        if (recordStatus.status == true)
                        {
                            return models;
                        }
        ...
        public async Task<DesignationModel> UpdateCacheAsync(DesignationModel modelRecord, string clientId, bool UseCache)
        {
