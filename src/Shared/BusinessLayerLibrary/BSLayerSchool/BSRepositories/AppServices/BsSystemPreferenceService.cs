...
namespace BSLayerSchool.BSRepositories.AppServices;

public sealed class BsSystemPreferencesService : BaseBusinessLayer, IBsSystemPreferencesContract
{
    private readonly IDataLayerSystemPreferencesContract _dlService;

    public BsSystemPreferencesService(IDataLayerSystemPreferencesContract dlService, ICacheContract _cache, IHttpContextAccessor httpContextAccessor, IMapper iMapper, ITrace trace)
    ...
    public async Task<ResponseDto<SystemPreferencesDtoModel>> Get(string id, bool UseCache = true)
    {
        ...
        ResponseDto<SystemPreferencesDtoModel> responseDto = new(_httpContextAccessor);
        ...
        List<SystemPreferencesModel>? modelRecords = new();
        SystemPreferencesModel? modelRecord = new SystemPreferencesModel();
        ...
        if (UseCache)
        {
            ...
            modelRecords = await _cacheData.ReadFromCacheAsync<SystemPreferencesModel>("Index", _modificationInDays);
        ...
        if (modelRecord == null)
        {
            ...
            if (modelRecord != null)
            {
                modelRecords = new List<SystemPreferencesModel>();
                ...
                await CacheService.SetDataAsync(_cacheData.GenerateKeyForCache<SystemPreferencesModel>("Index", _clientId), modelRecords, _expirationTime);
        ...
        //var modelRecord = await _dlService.Get(id);
        ...
        responseDto = new(_httpContextAccessor)
        {
            ...
            TimeConsumption = 0,
            Result = _mapper.Map<SystemPreferencesDtoModel>(modelRecord)
        };
        return await Task.Run(() => responseDto);
    }

    public async Task<ResponseDto<List<SystemPreferencesDtoModel>>> GetAll(int pageNo, int pageSize,
        bool UseCache = true)
    {
        ...
        if (modelRecords == null || modelRecords?.Count() == 0)
        {
            ...
            if (modelRecords != null && modelRecords.Count() > 0)
            {
                _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, $"{modelRecords?.Count()} records found from database server!".MarkProcess());
                await CacheService.SetDataAsync(_cacheData.GenerateKeyForCache<SystemPreferencesModel>("Index", _clientId), modelRecords, _expirationTime);
                _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, "Records saved in cache server!".MarkProcess());
            }
        }

        ResponseDto<List<SystemPreferencesDtoModel>>? responseDto = new(_httpContextAccessor);
        ...
        if (modelRecords == null || modelRecords?.Count() == 0)
        {
            ...
            responseDto.Result = _mapper.Map<List<SystemPreferencesDtoModel>>(modelRecords);
        ...
        responseDto = new(_httpContextAccessor)
        {
            ...
            Status = Status.Success,
            StatusCode = StatusCodes.Status200OK,
            Result = _mapper.Map<List<SystemPreferencesDtoModel>>(modelRecords),
    ...
    public async Task<ResponseDto<SystemPreferencesDtoModel>> AddAsync(SystemPreferencesDtoModel dto, bool UseCache = true)
    {
        ...
        if (await _dlService.FindExistingRecordAsync(dto.PreferenceName, dto.Id))
        {
            return DuplicateEntryResponse<SystemPreferencesDtoModel>("Preference Name", dto.PreferenceName);
        }
        //Do all business operation here
        var modelRecord = new SystemPreferencesModel();
        modelRecord = _mapper.Map<SystemPreferencesModel>(dto);
        ...
        var cacheResult = await UpdateCacheEntry<SystemPreferencesModel>(modelRecord, isNew: true);
    ...
    public async Task<ResponseDto<SystemPreferencesDtoModel>> UpdateAsync(string id, SystemPreferencesDtoModel dto,
        bool UseCache = true)
    {
        ...
        _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, OperationStart.MarkProcess(), $"Checking records for id {id}");
        SystemPreferencesModel? modelRecord = await _dlService.GetAsync(id);
        ...
        //Updating cache
        var cacheResult = await UpdateCacheEntry<SystemPreferencesModel>(modelRecord, isNew: false);
        if (cacheResult.status == false)
        {
            await GetAll(1, _resultPerPage, true);
        }
    ...
    public async Task<ResponseDto<SystemPreferencesDtoModel>> DeleteAsync(string id, bool UseCache = true)
    {
        ...
        if (UseCache)
        {
            DeleteCacheEntry<SystemPreferencesDtoModel>(id);
        }
