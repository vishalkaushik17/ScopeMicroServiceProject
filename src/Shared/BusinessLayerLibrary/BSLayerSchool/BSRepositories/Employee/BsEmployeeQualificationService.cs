...
{
    public class BsEmployeeQualificationService : BaseBusinessLayer, IBsEmployeeQualificationContract
    {
        private readonly IMCEmployeeQualificationContract _cacheService;

        public BsEmployeeQualificationService(IMCEmployeeQualificationContract cacheService, ICacheContract _cache, IHttpContextAccessor httpContextAccessor, IMapper iMapper, ITrace trace)
        ...
        public async Task<ResponseDto<EmployeeQualificationDtoModel>> Get(string id, bool UseCache = true)
        {
            ...
            EmployeeQualificationModel? modelRecord = await _cacheService.GetCacheAsync(id, _modificationInDays, _clientId, UseCache);

            if (modelRecord == null)
            {
                return FailureResponse<EmployeeQualificationDtoModel>(CommonMessages.RecordNotFound);
            }


            //Send response to controller class
            return await Task.Run(() => SuccessResponse<EmployeeQualificationModel, EmployeeQualificationDtoModel>(id, modelRecord));
        }

        public async Task<ResponseDto<List<EmployeeQualificationDtoModel>>> GetAll(int pageNo, int pageSize,
            bool UseCache = true)
        {
            _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, OperationStart.MarkProcess(), "Process started".MarkInformation());

            //First check cache entry
            List<EmployeeQualificationModel>? modelRecords = new List<EmployeeQualificationModel>();
            ...
            {
                return FailureResponse<List<EmployeeQualificationDtoModel>>(CommonMessages.RecordNotFound);
            }

            return await Task.Run(() => SuccessResponse<EmployeeQualificationModel, List<EmployeeQualificationDtoModel>>(modelRecords));
        }

        public async Task<ResponseDto<EmployeeQualificationDtoModel>> AddAsync(EmployeeQualificationDtoModel dto, bool UseCache = true)
        {
            ...
            if (await FindExistingRecordAsync(dto.EmployeeParentMaster.UIDNo, dto.Id, _modificationInDays, _clientId, UseCache))
            {
                return DuplicateEntryResponse<EmployeeQualificationDtoModel>("Uid No", dto.EmployeeParentMaster.UIDNo);
            }

            //Do all business operation here
            var modelRecord = new EmployeeQualificationModel();

            modelRecord = _mapper.Map<EmployeeQualificationModel>(dto);


            modelRecord = await _cacheService.AddCacheAsync(modelRecord, _clientId, UseCache);
            if (string.IsNullOrWhiteSpace(modelRecord.Id))
            {
                return FailureResponse<EmployeeQualificationDtoModel>(CommonMessages.OperationFailed);
            }
            modelRecord.CheckIsEditable(_modificationInDays);

            return await Task.Run(() => SuccessResponse<EmployeeQualificationModel, EmployeeQualificationDtoModel>(modelRecord.Id, modelRecord));
        }

        public async Task<ResponseDto<EmployeeQualificationDtoModel>> UpdateAsync(string id, EmployeeQualificationDtoModel dto,
            bool UseCache = true)
        {
            ...
            if (await FindExistingRecordAsync(dto.EmployeeParentMaster.UIDNo, dto.Id, _modificationInDays, _clientId, UseCache))
            {
                return DuplicateEntryResponse<EmployeeQualificationDtoModel>("Uid Name", dto.EmployeeParentMaster.UIDNo);
            }
            ...
            {
                //watch.Stop();
                return FailureResponse<EmployeeQualificationDtoModel>(CommonMessages.NotPermitted);
            }
        ...
        public async Task<ResponseDto<EmployeeQualificationDtoModel>> DeleteAsync(string id, bool UseCache = true)
        {
            ...
            {
                FailureResponse<EmployeeQualificationDtoModel>(CommonMessages.NotPermitted);
            }
            ...
            if (recordUsed)
            {
                return FailureResponse<EmployeeQualificationDtoModel>(CommonMessages.RecordInUsed);
            }
        ...
        {
            List<EmployeeQualificationModel>? records = await _cacheService.GetAllCacheAsync(_modificationInDays, _clientId, useCache);
            ...
            if (string.IsNullOrWhiteSpace(id))
            {
                ...
                return await Task.Run(() => records.Any(m => m.EmployeeParentMaster.UIDNo == fieldName
