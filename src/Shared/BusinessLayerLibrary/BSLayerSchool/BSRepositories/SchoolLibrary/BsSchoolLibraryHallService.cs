...
{
    public sealed class BsSchoolLibraryHallService : BaseBusinessLayer, IBsSchoolLibraryHallContract
    {
        private readonly IDataLayerSchoolLibraryHallContract _dlService;

        public BsSchoolLibraryHallService(IDataLayerSchoolLibraryHallContract dlService, ICacheContract _cache, IHttpContextAccessor httpContextAccessor, IMapper iMapper, ITrace trace)
        ...
        public async Task<ResponseDto<SchoolLibraryHallDtoModel>> Get(string id, bool UseCache = true)
        {
            ...
            ResponseDto<SchoolLibraryHallDtoModel> responseDto = new(_httpContextAccessor);
            ...
            List<SchoolLibraryHallModel>? modelRecords = new List<SchoolLibraryHallModel>();
            SchoolLibraryHallModel? modelRecord = new();
            ...
            if (UseCache)
            {
                ...
                modelRecords = await _cacheData.ReadFromCacheAsync<SchoolLibraryHallModel>("Index", _modificationInDays);
            ...
            if (modelRecord == null)
            {
                ...
                if (modelRecord != null)
                {
                    modelRecords = new List<SchoolLibraryHallModel>();
                    ...
                    await CacheService.SetDataAsync(_cacheData.GenerateKeyForCache<SchoolLibraryHallModel>("Index", _clientId), modelRecords, _expirationTime);
            ...
            responseDto = new(_httpContextAccessor)
            {
                ...
                TimeConsumption = 0,
                Result = _mapper.Map<SchoolLibraryHallDtoModel>(modelRecord)
            };
            return await Task.Run(() => responseDto);
        }

        public async Task<ResponseDto<List<SchoolLibraryHallDtoModel>>> GetAll(int pageNo, int pageSize,
            bool UseCache = true)
        {
            ...
            ResponseDto<List<SchoolLibraryHallDtoModel>>? responseDto = new(_httpContextAccessor);
            ...
            if (modelRecords == null || modelRecords?.Count() == 0)
            {
                ...
                responseDto.Result = _mapper.Map<List<SchoolLibraryHallDtoModel>>(modelRecords);
            ...
            responseDto = new(_httpContextAccessor)
            {
                ...
                Status = Status.Success,
                StatusCode = StatusCodes.Status200OK,
                Result = _mapper.Map<List<SchoolLibraryHallDtoModel>>(modelRecords),
        ...
        public async Task<ResponseDto<SchoolLibraryHallDtoModel>> AddAsync(SchoolLibraryHallDtoModel dto, bool UseCache = true)
        {
            ...
            {
                return DuplicateEntryResponse<SchoolLibraryHallDtoModel>("Hall Name", dto.Name);
            }
            //Do all business operation here
            var modelRecord = new SchoolLibraryHallModel();
            modelRecord = _mapper.Map<SchoolLibraryHallModel>(dto);
            modelRecord.Save(_userId);
            _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, OperationStart.MarkProcess(), $"Adding record <strong>Library Name : {modelRecord.Name} </strong>".MarkInformation());
            ...
            var cacheResult = await UpdateCacheEntry<SchoolLibraryHallModel>(modelRecord, isNew: true);
        ...
        public async Task<ResponseDto<SchoolLibraryHallDtoModel>> UpdateAsync(string id, SchoolLibraryHallDtoModel dto,
            bool UseCache = true)
        {
            ...
            if (await _dlService.FindExistingRecordAsync(dto.Name, dto.Id))
            {
                return DuplicateEntryResponse<SchoolLibraryHallDtoModel>("Name", dto.Name);
            }

            //check record availability by id
            _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, OperationStart.MarkProcess(), $"Checking records for id {id}");
            SchoolLibraryHallModel? modelRecord = await _dlService.GetAsync(id);
            ...
            //Updating cache
            var cacheResult = await UpdateCacheEntry<SchoolLibraryHallModel>(modelRecord, isNew: false);
            if (cacheResult.status == false)
            {
                await GetAll(1, _resultPerPage, true);
            }
        ...
        public async Task<ResponseDto<SchoolLibraryHallDtoModel>> DeleteAsync(string id, bool UseCache = true)
        {
            ...
            if (UseCache)
            {
                DeleteCacheEntry<SchoolLibraryHallModel>(id);
            }
