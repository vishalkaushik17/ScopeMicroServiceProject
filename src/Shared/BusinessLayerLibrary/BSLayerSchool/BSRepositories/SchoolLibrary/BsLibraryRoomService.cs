...
{
    public sealed class BsLibraryRoomService : BaseBusinessLayer, IBsLibraryRoomContract
    {
        private readonly IDataLayerLibraryRoomContract _dlService;

        public BsLibraryRoomService(IDataLayerLibraryRoomContract dlService, ICacheContract _cache, IHttpContextAccessor httpContextAccessor, IMapper iMapper, ITrace trace)
        ...
        public async Task<ResponseDto<LibraryRoomDtoModel>> Get(string id, string libraryId, bool UseCache = true)
        {
            ...
            ResponseDto<LibraryRoomDtoModel> responseDto = new(_httpContextAccessor);
            ...
            List<LibraryRoomModel>? modelRecords = new List<LibraryRoomModel>();
            LibraryRoomModel? modelRecord = new();
            ...
            if (UseCache)
            {
                ...
                modelRecords = await _cacheData.ReadFromCacheAsync<LibraryRoomModel>("Index", _modificationInDays);
            ...
            if (modelRecord == null)
            {
                ...
                modelRecord = await _dlService.GetAsync(id, libraryId);
                if (modelRecord != null)
                {
                    modelRecords = new List<LibraryRoomModel>();
                    ...
                    await CacheService.SetDataAsync(_cacheData.GenerateKeyForCache<LibraryRoomModel>("Index", _clientId), modelRecords, _expirationTime);
            ...
            responseDto = new(_httpContextAccessor)
            {
                ...
                TimeConsumption = 0,
                Result = _mapper.Map<LibraryRoomDtoModel>(modelRecord)
            };
            return await Task.Run(() => responseDto);
        }

        public async Task<ResponseDto<List<LibraryRoomDtoModel>>> GetAll(string libraryId, int pageNo, int pageSize,
            bool UseCache = true)
        {
            ...
            {
                _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, OperationStart.MarkProcess(), "Reading from Cache Server!".MarkInformation());
                modelRecords = await _cacheData.ReadFromCacheAsync<LibraryRoomModel>($"Index_", _modificationInDays);
                modelRecords = modelRecords?.Where(m => m.LibraryId == libraryId).ToList();
                _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, OperationEnd.MarkProcess(), $"Reading completed from Cache Server!".MarkInformation());
            }
            ...
            if (modelRecords == null || modelRecords?.Count() == 0)
            {
                ...
                modelRecords = await _dlService.GetAll();
                ...
                if (modelRecords != null && modelRecords.Count() > 0)
                {
                    ...
                    await CacheService.SetDataAsync(_cacheData.GenerateKeyForCache<LibraryRoomModel>($"Index_", _clientId), modelRecords, _expirationTime);
                    modelRecords = modelRecords?.Where(m => m.LibraryId == libraryId).ToList();
            ...
            ResponseDto<List<LibraryRoomDtoModel>>? responseDto = new(_httpContextAccessor);
            ...
            if (modelRecords == null || modelRecords?.Count() == 0)
            {
                ...
                responseDto.Result = null;//_mapper.Map<List<LibraryRoomDtoModel>>(modelRecords);
            ...
            responseDto = new(_httpContextAccessor)
            {
                ...
                Status = Status.Success,
                StatusCode = StatusCodes.Status200OK,
                Result = _mapper.Map<List<LibraryRoomDtoModel>>(modelRecords)
            };
            return await Task.Run(() => responseDto);
        }

        public async Task<ResponseDto<LibraryRoomDtoModel>> AddAsync(LibraryRoomDtoModel dto, bool UseCache = true)
        {
            ...
            if (await _dlService.FindExistingRecordAsync(dto.Name, dto.Id, dto.LibraryId))
            {
                return DuplicateEntryResponse<LibraryRoomDtoModel>("Library Room Name", dto.Name);
            }
            //Do all business operation here
            var modelRecord = new LibraryRoomModel();
            modelRecord = _mapper.Map<LibraryRoomModel>(dto);
            ...
            var cacheResult = await UpdateCacheEntry<LibraryRoomModel>(modelRecord, isNew: true);
            if (cacheResult.status == false)
            {
                await GetAll(modelRecord.LibraryId, 1, _resultPerPage, true);
            }
        ...
        public async Task<ResponseDto<LibraryRoomDtoModel>> UpdateAsync(string id, LibraryRoomDtoModel dto,
            bool UseCache = true)
        {
            ...
            if (await _dlService.FindExistingRecordAsync(dto.Name, dto.Id, dto.LibraryId))
            {
                return DuplicateEntryResponse<LibraryRoomDtoModel>("Name", dto.Name);
            }

            //check record availability by id
            LibraryRoomModel? modelRecord = await _dlService.GetAsync(id, dto.LibraryId);
            ...
            //Updating cache
            var cacheResult = await UpdateCacheEntry<LibraryRoomModel>(modelRecord, isNew: false);
            if (cacheResult.status == false)
            {
                await GetAll(modelRecord.LibraryId, 1, _resultPerPage, true);
            }
        ...
        public async Task<ResponseDto<LibraryRoomDtoModel>> DeleteAsync(string id, string libraryId, bool UseCache = true)
        {
            ...
            var modelRecord = await _dlService.GetAsync(id, libraryId);
            if (modelRecord == null)
            {
                //watch.Stop();
                _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, OperationEnd.MarkProcess(), "Process end. Record not available!!!");
            ...
            if (UseCache)
            {
                DeleteCacheEntry<LibraryRoomModel>(id);
            }
