...
{
    public sealed class BsLibraryBookCollectionService : BaseBusinessLayer, IBsLibraryBookCollectionContract
    {
        private readonly IDataLayerLibraryBookCollectionContract _dlService;

        public BsLibraryBookCollectionService(IDataLayerLibraryBookCollectionContract dlService, ICacheContract _cache, IHttpContextAccessor httpContextAccessor, IMapper iMapper, ITrace trace)
        ...
        public async Task<ResponseDto<LibraryBookCollectionDtoModel>> Get(string id, bool UseCache = true)
        {
            ...
            ResponseDto<LibraryBookCollectionDtoModel> responseDto = new(_httpContextAccessor);
            ...
            List<LibraryBookCollectionModel>? modelRecords = new List<LibraryBookCollectionModel>();
            LibraryBookCollectionModel? modelRecord = new();
            ...
            if (UseCache)
            {
                ...
                modelRecords = await _cacheData.ReadFromCacheAsync<LibraryBookCollectionModel>("Index", _modificationInDays);
            ...
            if (modelRecord == null)
            {
                ...
                if (modelRecord != null)
                {
                    modelRecords = new List<LibraryBookCollectionModel>();
                    ...
                    await CacheService.SetDataAsync(_cacheData.GenerateKeyForCache<LibraryBookCollectionModel>("Index", _clientId), modelRecords, _expirationTime);
            ...
            responseDto = new(_httpContextAccessor)
            {
                ...
                TimeConsumption = 0,
                Result = _mapper.Map<LibraryBookCollectionDtoModel>(modelRecord)
            };
            return await Task.Run(() => responseDto);
        }

        public async Task<ResponseDto<List<LibraryBookCollectionDtoModel>>> GetAll(int pageNo, int pageSize,
            bool UseCache = true)
        {
            ...
            ResponseDto<List<LibraryBookCollectionDtoModel>>? responseDto = new(_httpContextAccessor);
            ...
            if (modelRecords == null || modelRecords?.Count() == 0)
            {
                ...
                responseDto.Result = _mapper.Map<List<LibraryBookCollectionDtoModel>>(modelRecords);
            ...
            responseDto = new(_httpContextAccessor)
            {
                ...
                Status = Status.Success,
                StatusCode = StatusCodes.Status200OK,
                Result = _mapper.Map<List<LibraryBookCollectionDtoModel>>(modelRecords),
        ...
        public async Task<ResponseDto<LibraryBookCollectionDtoModel>> AddAsync(LibraryBookCollectionDtoModel dto, bool UseCache = true)
        {
            ...
            {
                return DuplicateEntryResponse<LibraryBookCollectionDtoModel>("Name", dto.Name);
            }
            //Do all business operation here
            var modelRecord = new LibraryBookCollectionModel();
            modelRecord = _mapper.Map<LibraryBookCollectionModel>(dto);
            ...
            //Add record to cache
            var cacheResult = await UpdateCacheEntry<LibraryBookCollectionModel>(modelRecord, isNew: true);
        ...
        public async Task<ResponseDto<LibraryBookCollectionDtoModel>> UpdateAsync(string id, LibraryBookCollectionDtoModel dto,
            bool UseCache = true)
        {
            ...
            if (await _dlService.FindExistingRecordAsync(dto.Name, dto.Id))
            {
                return DuplicateEntryResponse<LibraryBookCollectionDtoModel>("Collection Name", dto.Name);
            }

            //check record availability by id
            _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, OperationStart.MarkProcess(), $"Checking records for id {id}");
            LibraryBookCollectionModel? modelRecord = await _dlService.GetAsync(id);
            ...
            //Updating cache
            var cacheResult = await UpdateCacheEntry<LibraryBookCollectionModel>(modelRecord, isNew: false);
            if (cacheResult.status == false)
            {
                await GetAll(1, _resultPerPage, true);
            }
        ...
        public async Task<ResponseDto<LibraryBookCollectionDtoModel>> DeleteAsync(string id, bool UseCache = true)
        {
            ...
            if (UseCache)
            {
                DeleteCacheEntry<LibraryBookCollectionModel>(id);
            }
