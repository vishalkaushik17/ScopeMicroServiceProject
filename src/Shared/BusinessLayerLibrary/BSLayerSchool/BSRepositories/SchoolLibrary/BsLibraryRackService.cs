...
{
    public sealed class BsLibraryRackService : BaseBusinessLayer, IBsLibraryRackContract
    {
        private readonly IDataLayerLibraryRackContract _dlService;

        public BsLibraryRackService(IDataLayerLibraryRackContract dlService, ICacheContract _cache, IHttpContextAccessor httpContextAccessor, IMapper iMapper, ITrace trace)
        ...
        public async Task<ResponseDto<LibraryRackDtoModel>> Get(string id, string sectionId, bool UseCache = true)
        {
            ...
            ResponseDto<LibraryRackDtoModel> responseDto = new(_httpContextAccessor);
            ...
            List<LibraryRackModel>? modelRecords = new List<LibraryRackModel>();
            LibraryRackModel? modelRecord = new();
            ...
            if (UseCache)
            {
                ...
                modelRecords = await _cacheData.ReadFromCacheAsync<LibraryRackModel>("Index", _modificationInDays);
            ...
            if (modelRecord == null)
            {
                ...
                modelRecord = await _dlService.GetAsync(id, sectionId);
                if (modelRecord != null)
                {
                    modelRecords = new List<LibraryRackModel>();
                    ...
                    await CacheService.SetDataAsync(_cacheData.GenerateKeyForCache<LibraryRackModel>("Index", _clientId), modelRecords, _expirationTime);
            ...
            responseDto = new(_httpContextAccessor)
            {
                ...
                TimeConsumption = 0,
                Result = _mapper.Map<LibraryRackDtoModel>(modelRecord)
            };
            return await Task.Run(() => responseDto);
        }

        public async Task<ResponseDto<List<LibraryRackDtoModel>>> GetAll(string sectionId, int pageNo, int pageSize,
            bool UseCache = true)
        {
            ...
            {
                _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, OperationStart.MarkProcess(), "Reading from Database Server!".MarkInformation());
                modelRecords = await _dlService.GetAllAsync(sectionId);
            ...
            ResponseDto<List<LibraryRackDtoModel>>? responseDto = new(_httpContextAccessor);
            ...
            if (modelRecords == null || modelRecords?.Count() == 0)
            {
                ...
                responseDto.Result = _mapper.Map<List<LibraryRackDtoModel>>(modelRecords);
            ...
            responseDto = new(_httpContextAccessor)
            {
                ...
                Status = Status.Success,
                StatusCode = StatusCodes.Status200OK,
                Result = _mapper.Map<List<LibraryRackDtoModel>>(modelRecords)
            };
            return await Task.Run(() => responseDto);
        }
        ...
        public async Task<ResponseDto<LibraryRackDtoModel>> AddAsync(LibraryRackDtoModel dto, bool UseCache = true)
        {
            ...
            if (await _dlService.FindExistingRecordAsync(dto.Name, dto.Id, dto.SectionId))
            {
                return DuplicateEntryResponse<LibraryRackDtoModel>("Name", dto.Name);
            }
            //Do all business operation here
            var modelRecord = new LibraryRackModel();
            modelRecord = _mapper.Map<LibraryRackModel>(dto);
            ...
            //Add record to cache
            var cacheResult = await UpdateCacheEntry<LibraryRackModel>(modelRecord, isNew: true);
            if (cacheResult.status == false)
            {
                await GetAll(modelRecord.SectionId, 1, _resultPerPage, true);
            }
        ...
        public async Task<ResponseDto<LibraryRackDtoModel>> UpdateAsync(string id, LibraryRackDtoModel dto,
            bool UseCache = true)
        {
            ...
            if (await _dlService.FindExistingRecordAsync(dto.Name, dto.Id, dto.SectionId))
            {
                return DuplicateEntryResponse<LibraryRackDtoModel>("Rack Name", dto.Name);
            }

            //check record availability by id
            _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, OperationStart.MarkProcess(), $"Checking records for id {id}");
            LibraryRackModel? modelRecord = await _dlService.GetAsync(id, dto.SectionId);
            ...
            //Updating cache
            var cacheResult = await UpdateCacheEntry<LibraryRackModel>(modelRecord, isNew: false);
            if (cacheResult.status == false)
            {
                await GetAll(modelRecord.SectionId, 1, _resultPerPage, true);
            }
        ...
        public async Task<ResponseDto<LibraryRackDtoModel>> DeleteAsync(string id, string sectionId, bool UseCache = true)
        {
            ...
            var modelRecord = await _dlService.GetAsync(id, sectionId);
            if (modelRecord == null)
            {
                //watch.Stop();
                _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, OperationEnd.MarkProcess(), "Process end. Record not available!!!");
            ...
            if (UseCache)
            {
                DeleteCacheEntry<LibraryRackModel>(id);
            }
