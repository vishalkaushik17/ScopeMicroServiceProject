...
{
    public sealed class BsLibraryBookMasterService : BaseBusinessLayer, IBsLibraryBookMasterContract
    {
        private readonly IDataLayerLibraryBookMasterContract _dlService;

        public BsLibraryBookMasterService(IDataLayerLibraryBookMasterContract dlService, ICacheContract _cache, IHttpContextAccessor httpContextAccessor, IMapper iMapper, ITrace trace)
       : base(iMapper, httpContextAccessor, trace, _cache)
        ...
        public async Task<ResponseDto<LibraryBookMasterDtoModel>> Get(string id, bool UseCache = true)
        {
            ...
            ResponseDto<LibraryBookMasterDtoModel> responseDto = new(_httpContextAccessor);
            ...
            List<LibraryBookMasterModel>? modelRecords = new List<LibraryBookMasterModel>();
            LibraryBookMasterModel? modelRecord = new();
            ...
            if (UseCache)
            {
                ...
                modelRecords = await _cacheData.ReadFromCacheAsync<LibraryBookMasterModel>("Index", _modificationInDays);
            ...
            if (modelRecord == null)
            {
                ...
                if (modelRecord != null)
                {
                    modelRecords = new List<LibraryBookMasterModel>();
                    ...
                    await CacheService.SetDataAsync(_cacheData.GenerateKeyForCache<LibraryBookMasterModel>("Index", _clientId), modelRecords, _expirationTime);
            ...
            responseDto = new(_httpContextAccessor)
            {
                ...
                TimeConsumption = 0,
                Result = _mapper.Map<LibraryBookMasterDtoModel>(modelRecord)
            };
            return await Task.Run(() => responseDto);
        }

        public async Task<ResponseDto<List<LibraryBookMasterDtoModel>>> GetAll(int pageNo, int pageSize,
            bool UseCache = true)
        {
            ...
            ResponseDto<List<LibraryBookMasterDtoModel>>? responseDto = new(_httpContextAccessor);
            ...
            if (modelRecords == null || modelRecords?.Count() == 0)
            {
                ...
                responseDto.Result = _mapper.Map<List<LibraryBookMasterDtoModel>>(modelRecords);
            ...
            responseDto = new(_httpContextAccessor)
            {
                ...
                Status = Status.Success,
                StatusCode = StatusCodes.Status200OK,
                Result = _mapper.Map<List<LibraryBookMasterDtoModel>>(modelRecords),
        ...
        public async Task<ResponseDto<LibraryBookMasterDtoModel>> AddAsync(LibraryBookMasterDtoModel dto, bool UseCache = true)
        {
            ...
            if (await _dlService.FindExistingRecordAsync(dto.Title, dto.Id))
            {
                return DuplicateEntryResponse<LibraryBookMasterDtoModel>("Name", dto.Title);
            }
            //Do all business operation here
            var modelRecord = new LibraryBookMasterModel();
            modelRecord = _mapper.Map<LibraryBookMasterModel>(dto);
            ...
            //Add record to cache
            var cacheResult = await UpdateCacheEntry<LibraryBookMasterModel>(modelRecord, isNew: true);
        ...
        public async Task<ResponseDto<LibraryBookMasterDtoModel>> UpdateAsync(string id, LibraryBookMasterDtoModel dto,
            bool UseCache = true)
        {
            ...
            _trace.AddTrace(this.NameOfClass(), ExtensionMethods.GetCurrentMethodName(), _IsTracingRequired, OperationStart.MarkProcess(), $"Checking records for id {id}");
            LibraryBookMasterModel? modelRecord = await _dlService.GetAsync(id);
            ...
            //Updating cache
            var cacheResult = await UpdateCacheEntry<LibraryBookMasterModel>(modelRecord, isNew: false);
            if (cacheResult.status == false)
            {
                await GetAll(1, _resultPerPage, true);
            }
        ...
        public async Task<ResponseDto<LibraryBookMasterDtoModel>> DeleteAsync(string id, bool UseCache = true)
        {
            ...
            if (UseCache)
            {
                DeleteCacheEntry<LibraryBookMasterModel>(id);
            }
