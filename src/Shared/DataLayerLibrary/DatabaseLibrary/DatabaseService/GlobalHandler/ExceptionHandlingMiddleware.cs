...
public sealed class ExceptionHandlingMiddleware : IMiddleware
{
      ...
      IEmailService _emailService, ITrace trace, IConfiguration configuration)
    {
        ...
        var routeValues = ((dynamic)context.Request).RouteValues as IReadOnlyDictionary<string, object>;
        ...
        //if (Enumerable.Count<Claim>(httpContextAccessor?.HttpContext?.User.Claims) > 0)
        //{
        //    userId = Enumerable.FirstOrDefault<Claim>(httpContextAccessor.HttpContext?.User?.Claims, c => c.Type == "UserId").Value;
        //    clientId = httpContextAccessor?.HttpContext?.User?.Claims?.FirstOrDefault(c => c.Type == "ClientId").Value;
        //    userName = httpContextAccessor?.HttpContext?.User?.Claims?.FirstOrDefault(c => c.Type == "UserName").Value;
        //    clientName = httpContextAccessor?.HttpContext?.User?.Claims?.FirstOrDefault(c => c.Type == "ClientName").Value;


        //}
        //if (string.IsNullOrWhiteSpace(userId) || string.IsNullOrWhiteSpace(clientId))
        //{
        userId = httpContextAccessor?.HttpContext?.GetHeader(ContextKeys.UserId);
        clientId = httpContextAccessor?.HttpContext?.GetHeader(ContextKeys.ClientId);
        userName = httpContextAccessor?.HttpContext?.GetHeader(ContextKeys.UserName);
        clientName = httpContextAccessor?.HttpContext?.GetHeader(ContextKeys.ClientName);
        ...
        var clientResponse = new ResponseDto<object>(httpContextAccessor)
        {
