...
public sealed class DLLibraryBookCollectionService : BaseGenericRepository<LibraryBookCollectionModel>, IDataLayerLibraryBookCollectionContract
{
    #region Constructor
    public DLLibraryBookCollectionService(IHttpContextAccessor httpContextAccessor, ApplicationDbContext dbContext,
        IMapper iMapper, ITrace trace) :
    ...
    /// <returns></returns>
    public async Task<List<LibraryBookCollectionModel>?> GetAllAsync()
    {

        var modelRecordsFromDb = await _dbContext.LibraryBookCollections.Where(m => m.RecordStatus == EnumRecordStatus.Active).ToListAsync();
    ...
    #region Get Method
    public async Task<LibraryBookCollectionModel?> GetAsync(string id)
    {
        var record = await _dbContext.LibraryBookCollections.FirstOrDefaultAsync
    ...
    {
        var records = await _dbContext.LibraryBookCollections.Where(m => m.RecordStatus == EnumRecordStatus.Active).ToListAsync();
    ...
    public async Task<LibraryBookCollectionModel> AddAsync(LibraryBookCollectionModel modelRecord)
    {
        //Save data to db
    ...
    public async Task<LibraryBookCollectionModel> UpdateAsync(LibraryBookCollectionModel modelRecord)
    {
    ...
    {
        return await _dbContext.LibraryBookMasters.AnyAsync(m => m.RecordStatus == EnumRecordStatus.Active && m.CollectionId == id);
    }
}