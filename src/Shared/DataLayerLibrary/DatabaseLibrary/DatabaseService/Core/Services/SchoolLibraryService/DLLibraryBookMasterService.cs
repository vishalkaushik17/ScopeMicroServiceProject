...
public sealed class DLLibraryBookMasterService : BaseGenericRepository<LibraryBookMasterModel>,
    IDataLayerLibraryBookMasterContract

{

    #region Constructor
    public DLLibraryBookMasterService(IHttpContextAccessor httpContextAccessor, ApplicationDbContext dbContext, ITrace trace) :
    ...
    /// <returns></returns>
    public async Task<List<LibraryBookMasterModel>?> GetAllAsync()
    {

        var modelRecordsFromDb = await _dbContext.LibraryBookMasters.Where(m => m.RecordStatus == EnumRecordStatus.Active).ToListAsync();
    ...
    #region Get Method
    public async Task<LibraryBookMasterModel?> GetAsync(string id)
    {
        var record = await _dbContext.LibraryBookMasters.FirstOrDefaultAsync
    ...
    {
        var records = await _dbContext.LibraryBookMasters.Where(m => m.RecordStatus == EnumRecordStatus.Active).ToListAsync();
        ...
        if (string.IsNullOrWhiteSpace(id))
        {
            ...
            return await Task.Run(() => records.Any(m => m.Title.ToUpper() == name.ToUpper()
    ...
    public async Task<LibraryBookMasterModel> AddAsync(LibraryBookMasterModel modelRecord)
    {
        //Save data to db
        await _dbContext.LibraryBookMasters.AddAsync(modelRecord);
    ...
    public async Task<LibraryBookMasterModel> UpdateAsync(LibraryBookMasterModel modelRecord)
    {
