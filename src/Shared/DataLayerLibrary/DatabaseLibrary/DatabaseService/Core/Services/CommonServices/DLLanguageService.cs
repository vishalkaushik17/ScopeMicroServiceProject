...
public sealed class DLLanguageService : BaseGenericRepository<LanguageModel>,
    IDataLayerLanguageContract

{

    #region Constructor
    public DLLanguageService(IHttpContextAccessor httpContextAccessors, ApplicationDbContext dbContext, ITrace trace) :
    ...
    public async Task<List<LanguageModel>?> GetAllAsync()
    {
        ...
        var modelRecordsFromDb = await _dbContext.Languages.Where(m => m.RecordStatus == EnumRecordStatus.Active).ToListAsync();
    ...
    #region Get Method
    public async Task<LanguageModel?> GetAsync(string id)
    {
        ...
        LanguageModel? record = await _dbContext.Languages.FirstOrDefaultAsync
    ...
    public async Task<LanguageModel> AddAsync(LanguageModel modelRecord)
    {
        ...
        using (var transactionScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
        {
            try
            {
                ...
                await _dbContext.Languages.AddAsync(modelRecord);
                await _dbContext.SaveChangesAsync();

                modelRecord = await _dbContext.Languages.FirstOrDefaultAsync(m => m.Name == modelRecord.Name && m.CreatedOn == modelRecord.CreatedOn);

                //when no record found for above condition return back.
                if (modelRecord == null)
                {
                    transactionScope.Dispose();
                    return await Task.Run(() => new LanguageModel());
                }
    ...
    public async Task<LanguageModel> UpdateAsync(LanguageModel modelRecord)
    {
        try
        {
    ...
    public async Task<LanguageModel> DeleteAsync(LanguageModel modelRecord)
    {
        try
        {
