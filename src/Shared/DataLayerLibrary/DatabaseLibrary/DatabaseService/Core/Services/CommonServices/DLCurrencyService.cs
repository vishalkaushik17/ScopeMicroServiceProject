...
public sealed class DLCurrencyService : BaseGenericRepository<CurrencyModel>,
    IDataLayerCurrencyContract

{

    #region Constructor
    public DLCurrencyService(IHttpContextAccessor httpContextAccessors, ApplicationDbContext dbContext, ITrace trace) :
    ...
    public async Task<List<CurrencyModel>?> GetAllAsync()
    {
        ...
        var modelRecordsFromDb = await _dbContext.Currencies.Where(m => m.RecordStatus == EnumRecordStatus.Active).ToListAsync();
    ...
    #region Get Method
    public async Task<CurrencyModel?> GetAsync(string id)
    {
        ...
        CurrencyModel? record = await _dbContext.Currencies.FirstOrDefaultAsync
    ...
    public async Task<CurrencyModel> AddAsync(CurrencyModel modelRecord)
    {
        ...
        using (var transactionScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
        {
            try
            {
                ...
                await _dbContext.Currencies.AddAsync(modelRecord);
                await _dbContext.SaveChangesAsync();
                modelRecord = await _dbContext.Currencies.FirstOrDefaultAsync(m => m.Name == modelRecord.Name && m.CreatedOn == modelRecord.CreatedOn);
                ...
                if (modelRecord == null)
                {
                    ...
                    return await Task.Run(() => new CurrencyModel());
                }
    ...
    public async Task<CurrencyModel> UpdateAsync(CurrencyModel modelRecord)
    {
        try
        {
    ...
    public async Task<CurrencyModel> DeleteAsync(CurrencyModel modelRecord)
    {
        try
        {
