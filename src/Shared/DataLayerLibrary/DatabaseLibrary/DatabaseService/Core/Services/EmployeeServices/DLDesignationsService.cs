...
public sealed class DLDesignationsService : BaseGenericRepository<DesignationModel>, IDataLayerDesignationContract

{

    #region Constructor
    public DLDesignationsService(IHttpContextAccessor httpContextAccessor, ApplicationDbContext dbContext, ITrace trace) :
    ...
    public async Task<List<DesignationModel>?> GetAllAsync()
    {
        ...
        var modelRecordsFromDb = await _dbContext.DesignationMaster.Include(o => o.Department).Where(m => m.RecordStatus == EnumRecordStatus.Active).ToListAsync();
        if (modelRecordsFromDb?.Count() > 0)
        {
            ...
            modelRecordsFromDb?.ToList().ForEach(
               u => u.Department.Designations = null
           );
        }
    ...
    #region Get Method
    public async Task<DesignationModel?> GetAsync(string id)
    {
        var record = await _dbContext.DesignationMaster.Include(o => o.Department).FirstOrDefaultAsync
        ...
        record.Department.Designations = null;
        return record;
    ...
    public async Task<DesignationModel> AddAsync(DesignationModel modelRecord)
    {
        try
        {
            ...
            modelRecord = await _dbContext.DesignationMaster.Include(i => i.Department).FirstOrDefaultAsync(m => m.Name == modelRecord.Name &&
                m.CreatedOn == modelRecord.CreatedOn);
            if (modelRecord != null)
            {
                modelRecord.Department.Designations = null;
            }
    ...
    public async Task<DesignationModel> UpdateAsync(DesignationModel modelRecord)
    {
        try
        {
            ...
            await _dbContext.SaveChangesAsync();
            modelRecord = await _dbContext.DesignationMaster.Include(i => i.Department).FirstOrDefaultAsync(m => m.Id == modelRecord.Id);
            if (modelRecord != null)
            {
                modelRecord.Department.Designations = null;
            }
    ...
    public async Task<DesignationModel> DeleteAsync(DesignationModel modelRecord)
    {
        try
        {
    ...
    /// <summary>
    /// Checking record use in employee table 
    /// </summary>
    /// <param name="id">Designation Id</param>
    /// <returns></returns>
    public async Task<bool> IsRecordInUseAsync(string id)
    {
        return await _dbContext.EmployeeStudentParentMaster.AnyAsync(m => m.RecordStatus == EnumRecordStatus.Active && m.DesignationId == id);
    }
