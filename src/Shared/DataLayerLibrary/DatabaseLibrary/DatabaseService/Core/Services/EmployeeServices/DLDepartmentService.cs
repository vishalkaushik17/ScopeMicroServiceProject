...
public sealed class DLDepartmentService : BaseGenericRepository<DepartmentModel>,
    IDataLayerDepartmentContract

{

    #region Constructor
    public DLDepartmentService(IHttpContextAccessor httpContextAccessors, ApplicationDbContext dbContext, ITrace trace) :
    ...
    public async Task<List<DepartmentModel>?> GetAllAsync()
    {
        ...
        var modelRecordsFromDb = await _dbContext.DepartmentMaster.Where(m => m.RecordStatus == EnumRecordStatus.Active).ToListAsync();
    ...
    #region Get Method
    public async Task<DepartmentModel?> GetAsync(string id)
    {
        ...
        DepartmentModel? record = await _dbContext.DepartmentMaster.FirstOrDefaultAsync
    ...
    public async Task<DepartmentModel> AddAsync(DepartmentModel modelRecord)
    {
        ...
        using (var transactionScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
        {
            try
            {
                ...
                await _dbContext.DepartmentMaster.AddAsync(modelRecord);
                await _dbContext.SaveChangesAsync();
                modelRecord = await _dbContext.DepartmentMaster.FirstOrDefaultAsync(m => m.Name == modelRecord.Name && m.CreatedOn == modelRecord.CreatedOn);
                ...
                if (modelRecord == null)
                {
                    ...
                    return await Task.Run(() => new DepartmentModel());
                }
    ...
    public async Task<DepartmentModel> UpdateAsync(DepartmentModel modelRecord)
    {
        try
        {
    ...
    public async Task<DepartmentModel> DeleteAsync(DepartmentModel modelRecord)
    {
        try
        {
    ...
    //checking department use case in designation table.
    //here we can have multiple check for the department id in different table.
    public async Task<bool> IsRecordInUseAsync(string id)
    {

        return await _dbContext.DesignationMaster.AnyAsync(m => m.RecordStatus == EnumRecordStatus.Active && m.DepartmentId == id);
    }
