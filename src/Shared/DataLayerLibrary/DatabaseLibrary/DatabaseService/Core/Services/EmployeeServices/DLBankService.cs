...
public sealed class DLBankService : BaseGenericRepository<BankModel>, IDataLayerBankContract

{

    #region Constructor
    public DLBankService(IHttpContextAccessor httpContextAccessor, ApplicationDbContext dbContext, ITrace trace) :
    ...
    public async Task<List<BankModel>?> GetAllAsync()
    {
        ...
        var modelRecordsFromDb = await _dbContext.BankMaster.Where(m => m.RecordStatus == EnumRecordStatus.Active).ToListAsync();
    ...
    #region Get Method
    public async Task<BankModel?> GetAsync(string id)
    {
        var record = await _dbContext.BankMaster.FirstOrDefaultAsync
    ...
    public async Task<BankModel> AddAsync(BankModel modelRecord)
    {
        try
        {
            ...
            modelRecord = await _dbContext.BankMaster.FirstOrDefaultAsync(m => m.Name == modelRecord.Name &&
    ...
    public async Task<BankModel> UpdateAsync(BankModel modelRecord)
    {
        try
        {
    ...
    public async Task<BankModel> DeleteAsync(BankModel modelRecord)
    {
        try
        {
